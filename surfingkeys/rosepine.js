...
api.addSearchAlias('q', 'netflix', 'https://www.netflix.com/search?q=', 's', 'null', function(response) {
...
api.addSearchAlias('l', 'bilibili', 'https://search.bilibili.com/all?from_source=webtop_search&spm_id_from=333.1007&search_source=5&keyword=', 's', 'null', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
api.addSearchAlias('t', 'twitch', 'https://www.twitch.tv/search?term=', 's', 'null', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
api.addSearchAlias('a', 'huya', 'https://www.huya.com/search?hsk=', 's', 'null', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
api.addSearchAlias('c', 'douyu', 'https://www.douyu.com/search/?kw=', 's', 'null', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
api.addSearchAlias('z', 'weibo', 'https://s.weibo.com/weibo?q=', 's', 'null', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
api.addSearchAlias('f', 'onlyfans', 'https://onlyfans.com/search?type=posts&q=', 's', 'null', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
api.addSearchAlias('v', 'txvideo', 'https://v.qq.com/x/search/?q=', 's', 'null', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
api.addSearchAlias('r', 'googletrans', 'https://translate.google.com/?sl=auto&tl=zh-CN&op=translate&text=', 's', 'null', function(response) {
    var res = JSON.parse(response.text);
    return res.map(function(r){
        return r.phrase;
    });
...
const hintsCss =
  "font-size: 13pt; font-family: 'Hack Nerd Font Mono', 'Cascadia Code', 'Helvetica Neue', Helvetica, Arial, sans-serif; border: 0px; color: #e0def4 !important; background: #232136; background-color: #232136";
...
settings.theme = `
  ...
  body {
    ...
    font-family: "Hack Nerd Font Mono", "Cascadia Code", "Helvetica Neue", Helvetica, Arial, sans-serif;
    font-size: 12px;
  }
  #sk_omnibar {
    overflow: hidden;
  ...
  kbd {
    ...
    display: inline-block;
    padding: 3px 5px;
    font: 11px "Hack Nerd Font Mono", "Cascadia Code", "Helvetica Neue", Helvetica, Arial, sans-serif;
    line-height: 10px;
    vertical-align: middle;
